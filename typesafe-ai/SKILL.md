...
## Local additions (added by 书恩📚)

### Multi-key rotation client (reference implementation)

For high-throughput or resilient integrations, a self-managed API key pool with
rotation and exponential backoff lives at `references/typesafe-client.ts`. It is a
drop-in HTTP client for `POST https://api.typesafe.ai/v1/systemone` that:

- reads keys from `TYPESAFE_API_KEYS` (comma-separated) or a single `TYPESAFE_API_KEY`;
- rotates keys on `401 Unauthorized` and marks the bad key dead;
- retries `429 Too Many Requests` / `529 Overloaded` with exponential backoff;
- never stores keys — they stay in your environment / `.env`.
