---
name: gzh-html
description: "【公众号运营技能 · HTML 生成 gzh-html】将 Markdown 转为适配微信公众号的带内联样式 HTML，支持代码高亮、数学公式、PlantUML、脚注、提示块、信息图，以及外链转文末引用（--cite）。触发词：markdown to html、md 转 html、微信外链转底部引用。"
version: 1.56.1
metadata:
  openclaw:
    homepage: https://github.com/chenen1225/chenen-skills/tree/main/gzh-skills/gzh-html
    requires:
      anyBins:
        - bun
        - npx
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `.baoyu-skills/gzh-html/EXTEND.md` | Project |
| 2 | `${XDG_CONFIG_HOME:-$HOME/.config}/baoyu-skills/gzh-html/EXTEND.md` | XDG |
| 3 | `$HOME/.baoyu-skills/gzh-html/EXTEND.md` | User home |
...
3. `gzh-draft` EXTEND.md `default_theme` (cross-skill fallback)
...
Read `$HOME/.baoyu-skills/gzh-draft/EXTEND.md` if it exists and look for a `default_theme:` line. Use the value if present; otherwise fall through.
