...
- **带标注图解（diagram）**：用 gzh-image --role diagram 给**导读 + 每章**各做一张 16:9 带标注图解（process/loop/system 结构图），插到对应章后。
- **正文插图（illustration）**：暖色黏土风用 gzh-image --role illustration（gpt-image-2，16:9，中文清晰）；同样按**导读 + 每章**各做一张装饰性配图，插到对应位置（导语插图在导读后、各章插图在章后）。与封面共用同一份 config.json；用户审美偏好暖色黏土，否定纯卡通科技蓝紫。
